  ...
  if (nameInput.value !== "") {
    ...
    if (!listItem) {
      ...
      addAggettivi.addEventListener("click", () =>
        addAggettiviFunction(listItem, itemName, textNode, addAggettivi)
      );
...
function addAggettiviFunction(listItem, itemName, textNode, addAggettivi) {
  const aggettivi = prompt("Inserisci tre aggettivi separati da una virgola");
  if (aggettivi !== null && aggettivi !== "") {
    ...
    createModifyDeleteButtons(listItem, itemName, textNode);
  }
}

function createModifyDeleteButtons(listItem, itemName, textNode) {
  ...
  modifyAggettivi.addEventListener("click", () =>
    modifyAggettiviFunction(listItem, itemName, textNode)
  );
  ...
  deleteAggettivi.addEventListener("click", () =>
    deleteAggettiviFunction(listItem, itemName, textNode, deleteAggettivi)
  );
...
function modifyAggettiviFunction(listItem, itemName, textNode) {
  const currentUsername = localStorage.getItem("username");
  const addedByUsername = listItem.dataset.username;
  const existingAggettivi = listItem.dataset.aggettivi;

  if (existingAggettivi && currentUsername === addedByUsername) {
    ...
    if (newAggettivi !== null && newAggettivi !== "") {
      ...
      localStorage.setItem(itemName, newAggettivi);
    }
  } else if (!existingAggettivi) {
    alert("Devi prima aggiungere degli aggettivi");
  } else {
    alert("Puoi modificare solo gli aggettivi che hai aggiunto.");
  }
}

function deleteAggettiviFunction(
  listItem,
  itemName,
  textNode,
  deleteAggettivi
